...
parser.add_argument('--dropmix_rate',
                    type=float,
                    default=0.0,
                    help='fraction of training steps that skip MSDA')
...
def experiment_name_non_mnist(dataset=args.dataset,
                              ...
                              dropmix_rate=args.dropmix_rate,
                              job_id=args.job_id,
    ...
    if mixup_alpha:
        exp_name += '_m_alpha_' + str(mixup_alpha) + '_dropmix_rate_' + str(dropmix_rate)
    if box:
        exp_name += '_box'
    if graph:
...
def train(train_loader, model, optimizer, epoch, args, log, mp=None):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.train == 'vanilla' or (args.train == 'mixup' and np.random.rand() < args.dropmix_rate):
